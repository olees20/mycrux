...
The Supabase service-role key is consumed only inside `src/lib/supabase/admin.ts`. That module imports `server-only`, keeps the raw client private, validates token hashes, and exposes narrow guest/pass lookup operations. Future privileged operations must follow the same pattern. The key is reserved for verified Stripe webhooks, signed guest preregistration/waiver/pass exchanges, platform administration, and controlled maintenance.
