...
export default async function RouteHistoryAnalyticsPage({ params, searchParams }: { params: Promise<{gymSlug:string}>; searchParams: Promise<Search> }) {
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    <div className="mt-3"><p className="text-sm font-bold uppercase tracking-[.2em] text-[var(--muted)]">Immutable route revisions</p><h1 className="mt-2 text-4xl font-black">Historical route analytics</h1><p className="mt-3 max-w-4xl text-sm text-[var(--muted)]">Every route definition, lifecycle, wall, tag, and physical-hold modification is retained here, including routes that have since been archived.</p></div>
    <form className="mt-6 grid gap-3 rounded-2xl border bg-white p-5 sm:grid-cols-5"><label className="text-sm font-bold">From<input className="mt-1 w-full rounded-lg border p-2 font-normal" defaultValue={from} name="from" type="date"/></label><label className="text-sm font-bold">To<input className="mt-1 w-full rounded-lg border p-2 font-normal" defaultValue={to} name="to" type="date"/></label><label className="text-sm font-bold">Historical wall<select className="mt-1 w-full rounded-lg border p-2 font-normal" defaultValue={wall??""} name="wall"><option value="">All</option>{(walls??[]).map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label className="text-sm font-bold">Historical setter<select className="mt-1 w-full rounded-lg border p-2 font-normal" defaultValue={setter??""} name="setter"><option value="">All</option>{(memberships??[]).map((item)=><option key={item.profile_id} value={item.profile_id}>{item.profiles?.display_name}</option>)}</select></label><label className="text-sm font-bold">Discipline<select className="mt-1 w-full rounded-lg border p-2 font-normal" defaultValue={routeType??""} name="type"><option value="">All</option><option value="boulder">Boulder</option><option value="sport">Sport</option><option value="top_rope">Top rope</option><option value="trad">Trad</option><option value="training">Training</option></select></label><button className="min-h-11 rounded-lg border p-2 font-bold sm:col-span-5">Apply filters</button></form>
