...
export default async function StaffTeamPage({ params }: { params: Promise<{ gymSlug: string }> }) {
  ...
  const [{ data: memberships }, { data: invitations }] = await Promise.all([
    ...
    supabase.from("invitations").select("id,email,staff_role_id,status,expires_at,created_at").eq("gym_id", gym.id).order("created_at", { ascending: false }),
  ]);
  ...
  return <div className="mx-auto max-w-6xl"><p className="text-sm font-bold uppercase tracking-[0.2em] text-[var(--muted)]">Staff access</p><h1 className="mt-3 text-4xl font-black">Team and invitations</h1><p className="mt-3 max-w-3xl text-[var(--muted)]">Invite staff with expiring single-use links, assign operational roles, and suspend access without deleting audit history.</p><section className="mt-8 rounded-2xl border border-[var(--border)] bg-white p-5"><h2 className="text-xl font-black">Invite staff</h2><InviteStaffForm canAssignManager={canAssignManager} gymSlug={gym.slug} /></section><section className="mt-8"><h2 className="text-xl font-black">Current staff</h2><ul className="mt-4 grid gap-4 md:grid-cols-2">{(memberships ?? []).map((membership) => { const role = rolesById.get(membership.staff_role_id ?? "")?.key as StaffRoleKey | undefined; if (!role) return null; const manageable = membership.profile_id !== authData.user?.id && canManageStaffRole(actorRole, role); return <li className="rounded-2xl border border-[var(--border)] bg-white p-5" key={membership.id}><p className="font-bold">{profilesById.get(membership.profile_id)?.display_name ?? "Staff member"}</p><p className="mt-1 text-sm text-[var(--muted)]">{rolesById.get(membership.staff_role_id ?? "")?.name} · {membership.status}</p>{manageable ? <StaffAccessForm canAssignManager={canAssignManager} gymSlug={gym.slug} membershipId={membership.id} role={role} status={membership.status} /> : <p className="mt-3 text-xs text-[var(--muted)]">You cannot change this access assignment.</p>}</li>; })}</ul></section><section className="mt-8"><h2 className="text-xl font-black">Invitations</h2><ul className="mt-4 grid gap-4 md:grid-cols-2">{(invitations ?? []).map((invitation) => { const role = rolesById.get(invitation.staff_role_id ?? "")?.key as StaffRoleKey | undefined; const manageable = role ? canManageStaffRole(actorRole, role) : false; return <li className="rounded-2xl border border-[var(--border)] bg-white p-5" key={invitation.id}><p className="font-bold">{invitation.email}</p><p className="mt-1 text-sm text-[var(--muted)]">{rolesById.get(invitation.staff_role_id ?? "")?.name ?? "Staff"} · {invitation.status} · expires {new Date(invitation.expires_at).toLocaleDateString("en-GB")}</p>{invitation.status === "pending" && manageable ? <InvitationControls gymSlug={gym.slug} invitationId={invitation.id} /> : null}</li>; })}</ul></section></div>;
}
