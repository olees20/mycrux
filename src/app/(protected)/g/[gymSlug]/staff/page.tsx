...
function Kpi({ value, label, definition }: { value: number; label: string; definition: string }) {
  return <article className="app-panel p-4"><p className="text-3xl font-extrabold tabular-nums tracking-[-.04em]">{value}</p><h2 className="mt-1 text-sm font-bold">{label}</h2><p className="mt-1 text-xs leading-5 text-[var(--muted)]">{definition}</p></article>;
}

function Queue({ title, description, children }: { title: string; description: string; children: React.ReactNode }) {
  return <section className="app-panel p-4 sm:p-5"><h2 className="text-xl font-bold tracking-[-.02em]">{title}</h2><p className="mt-1 text-sm text-[var(--muted)]">{description}</p><div className="mt-4">{children}</div></section>;
}

function Empty({ children }: { children: React.ReactNode }) {
  return <p className="rounded-[var(--radius-md)] bg-emerald-50 p-4 text-sm font-semibold text-emerald-900">{children}</p>;
}
...
export default async function StaffPage({ params }: { params: Promise<{ gymSlug: string }> }) {
  ...
  const quickActions = [
    ...
    ...(gym.role === "owner" ? [["Floorplan", "floorplan"], ["Gym analytics", "analytics"], ["Plans & usage", "plans"], ["Billing", "billing"], ["Integrations", "integrations"], ["Export data", "export"], ["Privacy requests", "privacy"], ["Gym settings", "settings"]] : []),
  ];
  ...
  return <div className="mx-auto max-w-[var(--content)]">
    {gym.role === "owner" && !settings?.setup_completed_at ? <section className="mb-8 rounded-[var(--radius-panel)] border border-lime-300 bg-lime-50 p-6" aria-labelledby="setup-heading"><p className="app-eyebrow text-lime-900">Setup in progress</p><h2 className="mt-2 text-2xl font-black text-lime-950" id="setup-heading">Finish preparing {gym.name}</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-lime-950">Your progress is saved. Complete the remaining essentials so routes, staff, and member access start with the right defaults.</p><Link className="mt-5 inline-flex min-h-11 items-center rounded-[var(--radius-md)] bg-[var(--foreground)] px-5 text-sm font-bold text-[var(--surface)] focus:outline-none focus:ring-2 focus:ring-[var(--focus-ring)] focus:ring-offset-2" href={`/g/${gym.slug}/staff/setup?step=${settings?.setup_current_step ?? 1}`}>Continue setup</Link></section> : null}
    <div className="flex flex-wrap items-end justify-between gap-4"><div><p className="app-eyebrow text-[var(--muted)]">{roleName} command centre</p><h1 className="mt-2 text-4xl font-extrabold tracking-[-.035em]">Today at {gym.name}</h1><p className="mt-2 text-sm text-[var(--muted)]">{range.label} · {timezone}</p></div><div className="flex items-center gap-2">{primaryAction ? <Link className={buttonStyles()} href={`/g/${gym.slug}/staff/${primaryAction[1]}`}>{primaryAction[0]}</Link> : null}{secondaryActions.length ? <details className="relative"><summary className={buttonStyles({variant:"secondary",className:"list-none marker:content-none"})}>Quick actions</summary><div className="absolute right-0 z-20 mt-2 grid w-64 grid-cols-1 gap-1 rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] p-2 shadow-[var(--shadow-md)]">{secondaryActions.map(([label,path])=><Link className="rounded-[var(--radius-md)] px-3 py-2 text-sm font-semibold hover:bg-[var(--surface-subtle)]" href={`/g/${gym.slug}/staff/${path}`} key={path}>{label}</Link>)}</div></details> : null}</div></div>
