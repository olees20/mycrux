...
export default async function StaffAnnouncementsPage({ params }: { params: Promise<{ gymSlug: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <p className="text-sm font-bold uppercase tracking-[0.2em] text-[var(--muted)]">Communications</p><h1 className="mt-3 text-4xl font-black">Announcements</h1>
      <section className="mt-8 rounded-2xl border border-[var(--border)] bg-white p-5"><h2 className="mb-5 text-xl font-black">New announcement</h2><AnnouncementForm gymSlug={gym.slug} timezone={timezone} /></section>
      <section className="mt-8"><h2 className="text-xl font-black">History and schedule</h2><div className="mt-4 space-y-4">
        {announcements?.map((item) => <article className="rounded-2xl border border-[var(--border)] bg-white p-5" key={item.id}><div className="mb-4 flex flex-wrap items-center justify-between gap-2"><h3 className="font-black">{item.title}</h3><span className="rounded-full bg-stone-100 px-3 py-1 text-xs font-bold">{item.status} · {item.priority}</span></div><AnnouncementForm gymSlug={gym.slug} timezone={timezone} values={{ id:item.id,title:item.title,body:item.body,audience:item.audience,priority:item.priority,status:item.status,publishedAt:item.published_at,expiresAt:item.expires_at,isPinned:item.is_pinned }} />{item.status !== "archived" ? <form action={archiveAnnouncementAction} className="mt-3"><input name="gymSlug" type="hidden" value={gym.slug} /><input name="announcementId" type="hidden" value={item.id} /><button className="text-sm font-bold text-red-700">Archive announcement</button></form> : null}</article>)}
        {announcements?.length ? null : <p className="rounded-2xl bg-white p-5 text-sm text-[var(--muted)]">No announcements yet.</p>}
      </div></section>
    </div>
  );
}
