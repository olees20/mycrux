...
export default async function GymSettingsPage({ params, searchParams }: { params: Promise<{ gymSlug: string }>; searchParams: Promise<{ created?: string }> }) {
  ...
  return <div className="mx-auto max-w-5xl">
    {created ? <section className="mb-8 rounded-3xl border border-green-200 bg-green-50 p-6" role="status"><p className="text-sm font-bold uppercase tracking-[0.2em] text-green-800">Gym created</p><h1 className="mt-2 text-3xl font-black text-green-950">Welcome to {details.name}</h1><p className="mt-3 max-w-2xl leading-7 text-green-900">You are the gym owner. Complete the settings below, then publish member access or review your trial and billing options.</p><div className="mt-5 flex flex-wrap gap-3"><Link className={buttonStyles()} href={`/g/${gym.slug}/staff/member-access`}>Open member access</Link><Link className={buttonStyles({ variant: "secondary" })} href={`/g/${gym.slug}/staff/billing`}>Review billing</Link></div></section> : null}
    <p className="text-sm font-bold uppercase tracking-[0.2em] text-[var(--muted)]">Owner settings</p>
    {created ? <h2 className="mt-3 text-4xl font-black">Gym and branding</h2> : <h1 className="mt-3 text-4xl font-black">Gym and branding</h1>}
