...
export default async function GymSetupPage({ params, searchParams }: { params: Promise<{ gymSlug: string }>; searchParams: Promise<{ step?: string }> }) {
  ...
  return <div className="mx-auto max-w-5xl pb-12">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="app-eyebrow text-[var(--muted)]">First-time gym setup</p><h1 className="mt-2 text-4xl font-extrabold tracking-[-.035em]">Make {gym.name} ready</h1><p className="mt-3 max-w-2xl leading-7 text-[var(--muted)]">Each step is saved separately. You can return to the dashboard and resume at any time.</p></div><Link className={buttonStyles({ variant: "secondary" })} href={`/g/${gym.slug}/staff`}>Save and leave</Link></div>

    <nav aria-label="Setup progress" className="mt-8 overflow-x-auto"><ol className="flex min-w-max gap-2">{steps.map((label, index) => { const number = index + 1; const available = completed || number <= details.setup_current_step; return <li key={label}>{available ? <Link aria-current={number === step ? "step" : undefined} className={`inline-flex min-h-11 items-center rounded-full border px-4 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-[var(--focus-ring)] focus:ring-offset-2 ${number === step ? "border-[var(--primary)] bg-[var(--primary)] text-white" : "border-[var(--border)] bg-[var(--surface)]"}`} href={`/g/${gym.slug}/staff/setup?step=${number}`}>{number}. {label}</Link> : <span aria-disabled="true" className="inline-flex min-h-11 items-center rounded-full border border-[var(--border)] px-4 text-sm font-bold opacity-45">{number}. {label}</span>}</li>; })}</ol></nav>

    <section className="mt-8 rounded-[var(--radius-panel)] border border-[var(--border)] bg-[var(--surface)] p-5 sm:p-8">
      ...
      {step === 4 ? <><p className="text-sm font-bold uppercase text-[var(--muted)]">Step 4 of 6</p><h2 className="mt-2 text-3xl font-black">Create your first wall or area</h2><p className="mt-2 mb-7 text-[var(--muted)]">Routes belong to an existing wall, zone, or sector. Add at least one before continuing.</p><div className="space-y-4">{walls?.map((wall) => <div className="rounded-[var(--radius-lg)] bg-[var(--surface-subtle)] p-4" key={wall.id}><WallForm gymSlug={gym.slug} wall={wall} /></div>)}<div className="rounded-[var(--radius-lg)] border border-dashed border-[var(--border)] p-4"><WallForm gymSlug={gym.slug} /></div></div><div className="mt-7"><SetupContinueForm gymSlug={gym.slug} step={4} /></div></> : null}
      ...
      {step === 6 ? <><p className="text-sm font-bold uppercase text-[var(--muted)]">Step 6 of 6</p><h2 className="mt-2 text-3xl font-black">Ready to open CRUX</h2><p className="mt-2 text-[var(--muted)]">Review the minimum setup, then mark it complete.</p><dl className="mt-7 grid gap-4 sm:grid-cols-2"><div className="rounded-[var(--radius-lg)] bg-[var(--surface-subtle)] p-5"><dt className="text-sm font-bold text-[var(--muted)]">Location</dt><dd className="mt-1 font-black">{details.city}, {details.country_code} · {details.timezone}</dd></div><div className="rounded-[var(--radius-lg)] bg-[var(--surface-subtle)] p-5"><dt className="text-sm font-bold text-[var(--muted)]">Climbing</dt><dd className="mt-1 font-black">{details.disciplines.join(", ")}</dd></div><div className="rounded-[var(--radius-lg)] bg-[var(--surface-subtle)] p-5"><dt className="text-sm font-bold text-[var(--muted)]">Areas</dt><dd className="mt-1 font-black">{walls?.length ?? 0} active</dd></div><div className="rounded-[var(--radius-lg)] bg-[var(--surface-subtle)] p-5"><dt className="text-sm font-bold text-[var(--muted)]">Team</dt><dd className="mt-1 font-black">{staffCount ?? 0} additional staff member{staffCount === 1 ? "" : "s"}</dd></div></dl>{completed ? <div className="mt-7 rounded-[var(--radius-lg)] bg-emerald-50 p-5"><p className="font-black text-emerald-950">Setup is complete.</p><Link className={`${buttonStyles()} mt-4`} href={`/g/${gym.slug}/staff`}>Go to dashboard</Link></div> : <div className="mt-7"><SetupContinueForm complete gymSlug={gym.slug} step={6} /></div>}<aside className="mt-8 border-t border-[var(--border)] pt-7"><h3 className="font-black">Optional next steps</h3><ul className="mt-3 list-disc space-y-2 pl-5 text-sm text-[var(--muted)]"><li>Print the member QR and place it near reception.</li><li>Create routes and upload a current wall image.</li><li>Review plans, billing, waivers, and day-pass registration.</li></ul></aside></> : null}
    </section>
  </div>;
}
