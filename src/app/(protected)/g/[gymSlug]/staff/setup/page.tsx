...
import { InviteStaffForm } from "@/components/staff-team-controls";
...
export default async function GymSetupPage({ params, searchParams }: { params: Promise<{ gymSlug: string }>; searchParams: Promise<{ step?: string }> }) {
  ...
  const [{ data: details, error: detailsError }, { data: branding, error: brandingError }, { data: walls }, { count: invitationCount }] = await Promise.all([
    supabase.from("gyms").select("name,address_line_1,address_line_2,city,postcode,country_code,timezone,contact_email,contact_phone,disciplines,settings,public_join_requests_enabled,setup_current_step,setup_completed_at").eq("id", gym.id).single(),
    ...
    supabase.from("invitations").select("id", { count: "exact", head: true }).eq("gym_id", gym.id).eq("status", "pending"),
  ]);
  ...
  return <div className="mx-auto max-w-5xl pb-12">
    ...
    <section className="mt-8 rounded-3xl border border-[var(--border)] bg-white p-5 sm:p-8">
      ...
      {step === 5 ? <><p className="text-sm font-bold uppercase text-[var(--muted)]">Step 5 of 6</p><h2 className="mt-2 text-3xl font-black">Team and members</h2><p className="mt-2 text-[var(--muted)]">Staff invitations are expiring single-use links. Create one now or continue and invite people later.</p><InviteStaffForm canAssignManager gymSlug={gym.slug} /><div className="mt-8 border-t border-[var(--border)] pt-7"><SetupContinueForm gymSlug={gym.slug} step={5}><label className="flex items-start gap-3 rounded-2xl bg-stone-50 p-4 text-sm font-semibold"><input className="mt-1" defaultChecked={details.public_join_requests_enabled} name="publicJoinRequestsEnabled" type="checkbox" /><span>List this gym for authenticated users requesting membership.<span className="mt-1 block font-normal text-[var(--muted)]">CRUX currently supports invitation links and moderated public requests; it does not expose a reusable plaintext join code.</span></span></label></SetupContinueForm></div></> : null}
      {step === 6 ? <><p className="text-sm font-bold uppercase text-[var(--muted)]">Step 6 of 6</p><h2 className="mt-2 text-3xl font-black">Ready to open CRUX</h2><p className="mt-2 text-[var(--muted)]">Review the minimum setup, then mark it complete.</p><dl className="mt-7 grid gap-4 sm:grid-cols-2"><div className="rounded-2xl bg-stone-50 p-5"><dt className="text-sm font-bold text-[var(--muted)]">Location</dt><dd className="mt-1 font-black">{details.city}, {details.country_code} · {details.timezone}</dd></div><div className="rounded-2xl bg-stone-50 p-5"><dt className="text-sm font-bold text-[var(--muted)]">Climbing</dt><dd className="mt-1 font-black">{details.disciplines.join(", ")}</dd></div><div className="rounded-2xl bg-stone-50 p-5"><dt className="text-sm font-bold text-[var(--muted)]">Areas</dt><dd className="mt-1 font-black">{walls?.length ?? 0} active</dd></div><div className="rounded-2xl bg-stone-50 p-5"><dt className="text-sm font-bold text-[var(--muted)]">Team</dt><dd className="mt-1 font-black">{invitationCount ?? 0} pending invitation{invitationCount === 1 ? "" : "s"}</dd></div></dl>{completed ? <div className="mt-7 rounded-2xl bg-emerald-50 p-5"><p className="font-black text-emerald-950">Setup is complete.</p><Link className={`${buttonStyles()} mt-4`} href={`/g/${gym.slug}/staff`}>Go to dashboard</Link></div> : <div className="mt-7"><SetupContinueForm complete gymSlug={gym.slug} step={6} /></div>}<aside className="mt-8 border-t border-[var(--border)] pt-7"><h3 className="font-black">Optional next steps</h3><ul className="mt-3 list-disc space-y-2 pl-5 text-sm text-[var(--muted)]"><li>Add opening hours and a welcome message in gym settings.</li><li>Create routes and upload a current wall image.</li><li>Review plans, billing, waivers, and day-pass registration.</li></ul></aside></> : null}
    </section>
  </div>;
}
