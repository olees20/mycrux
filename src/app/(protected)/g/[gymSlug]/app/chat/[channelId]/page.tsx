...
export default async function ChatChannelPage({ params }: { params: Promise<{ gymSlug: string; channelId: string }> }) {
  ...
  return <div className="mx-auto max-w-4xl"><Link className="text-sm font-bold underline" href={`/g/${gym.slug}/app/chat`}>← All channels</Link><div className="my-5"><p className="text-xs font-bold uppercase tracking-[.2em] text-[var(--muted)]">{channel.channel_type === "partner" ? "Private partner thread" : "Gym chat"}</p><h1 className="mt-2 text-4xl font-black">{channel.name}</h1>{channel.description ? <p className="mt-2 text-[var(--muted)]">{channel.description}</p> : null}</div><ChatThread channelId={channel.id} currentUserId={user.id} gymId={gym.id} gymSlug={gym.slug} initialMessages={[...(messages ?? [])].reverse() as ChatMessage[]} readOnly={channel.is_read_only && gym.role === "member"}/></div>;
}
