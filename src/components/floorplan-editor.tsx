...
const toolClass = "min-h-11 rounded-xl border px-4 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-2 disabled:opacity-40";
...
export function FloorplanEditor({ configuration: initialConfiguration, floorplanId, gymSlug, initialFaces, initialFaceRevisions, initialRevision, initialWalls }: { configuration: FloorplanConfiguration; floorplanId: string; gymSlug: string; initialFaces: ClimbingFace[]; initialFaceRevisions: Record<string, number>; initialRevision: number; initialWalls: FloorplanWall[] }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_20rem]">
    <section className="min-w-0 overflow-hidden rounded-3xl border border-[var(--border)] bg-white shadow-sm" aria-labelledby="canvas-heading">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border)] p-3">
        <div className="flex flex-wrap gap-2" aria-label="Floorplan tools">
          {(["select", "draw", "pan"] as Tool[]).map((item) => <button aria-pressed={tool === item} className={`${toolClass} ${tool === item ? "border-black bg-black text-white" : "bg-white"}`} key={item} onClick={() => { setTool(item); setDraftStart(null); setPreview(null); }} type="button">{item === "select" ? "Select" : item === "draw" ? "Draw wall" : "Pan"}</button>)}
      ...
      <div className="relative h-[60vh] min-h-[28rem] bg-stone-100">
        ...
        {!walls.length && !draftStart ? <div className="pointer-events-none absolute inset-0 grid place-items-center p-8 text-center"><div className="max-w-sm rounded-2xl bg-white/90 p-6 shadow-sm"><p className="text-xl font-black">Start with your first wall</p><p className="mt-2 text-sm text-[var(--muted)]">Choose Draw wall, click its start point, then click its end point.</p></div></div> : null}
        <p className="absolute bottom-3 left-3 rounded-lg bg-white/90 px-3 py-2 text-xs font-bold shadow-sm" aria-live="polite">{draftWall ? `Drawing ${wallLength(draftWall).toFixed(2)} m` : coordinateLabel}</p>
      </div>
    ...
    <aside className="space-y-5">
      ...
      <section className="rounded-3xl border border-[var(--border)] bg-white p-5"><div className="flex items-center justify-between"><h2 className="text-xl font-black">Walls</h2><span className="text-sm font-bold text-[var(--muted)]">{walls.length}</span></div>{walls.length ? <ol className="mt-3 max-h-72 space-y-2 overflow-auto">{walls.map((wall) => <li key={wall.id}><button aria-current={selectedId === wall.id ? "true" : undefined} className={`flex min-h-12 w-full items-center justify-between rounded-xl px-3 text-left text-sm focus:outline-none focus:ring-2 focus:ring-black ${selectedId === wall.id ? "bg-lime-100" : "bg-stone-50"}`} onClick={() => { setSelectedId(wall.id); setTool("select"); }} type="button"><strong className="truncate">{wall.name}</strong><span className="ml-2 tabular-nums text-[var(--muted)]">{wallLength(wall).toFixed(2)} m</span></button></li>)}</ol> : <p className="mt-3 rounded-xl bg-stone-50 p-4 text-sm text-[var(--muted)]">No walls yet. The blank canvas is ready.</p>}</section>
    </aside>
  </div>;
}
