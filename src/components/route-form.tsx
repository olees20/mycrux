...
export function RouteForm({ gymSlug, walls, setters, route, defaults }: { gymSlug: string; walls: WallOption[]; setters: { id: string; label: string }[]; route?: RouteValue; defaults?: { gradeSystem: string; grade: string; routeType: RouteValue["route_type"] } }) {
  ...
  return <div className="rounded-2xl border border-[var(--border)] bg-white p-5">
    <form action={action} className="grid gap-4 md:grid-cols-2">
      ...
      <div className="grid grid-cols-2 gap-3"><label className="text-sm font-bold">Grade system<input className="mt-1 w-full rounded-lg border p-3 font-normal" defaultValue={route?.grade_system ?? defaults?.gradeSystem ?? "font"} maxLength={30} name="gradeSystem" required /></label><label className="text-sm font-bold">Grade<input className="mt-1 w-full rounded-lg border p-3 font-normal" defaultValue={route?.grade ?? defaults?.grade ?? "6A"} maxLength={20} name="grade" required /></label></div>
      <label className="text-sm font-bold">Discipline<select className="mt-1 w-full rounded-lg border p-3 font-normal" defaultValue={route?.route_type ?? defaults?.routeType ?? "boulder"} name="discipline"><option value="boulder">Boulder</option><option value="sport">Sport</option><option value="top_rope">Top rope</option><option value="trad">Trad</option><option value="training">Training</option></select></label>
