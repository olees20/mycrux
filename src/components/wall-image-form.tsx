...
export function WallImageForm({ gymSlug, wallId }: { gymSlug: string; wallId: string }) {
  ...
  return <form action={action} className="mt-4 grid gap-3 rounded-xl bg-stone-50 p-4 sm:grid-cols-2">
    ...
    <label className="text-sm font-bold sm:col-span-2">Accessible description<input className="mt-1 w-full rounded-lg border p-3 font-normal" maxLength={500} name="altText" required /></label>
    <label className="text-sm font-bold">Photo date<input className="mt-1 w-full rounded-lg border p-3 font-normal" name="capturedAt" type="date" /></label>
    <button className="min-h-11 self-end rounded-lg bg-black px-4 text-sm font-bold text-white disabled:opacity-50" disabled={pending || dimensions.width === 0}>{pending ? "Uploading…" : "Upload wall image"}</button>
