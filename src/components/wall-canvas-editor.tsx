...
const buttonClass = "min-h-11 rounded-[var(--radius-md)] border border-[var(--border)] px-4 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-[var(--focus-ring)] focus:ring-offset-2 disabled:opacity-40";
  ...
  return <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_22rem]">
    <section className="min-w-0 overflow-hidden rounded-[var(--radius-panel)] border border-[var(--border)] bg-[var(--surface)] shadow-sm" aria-labelledby="wall-canvas-heading">
      ...
      <div className="relative h-[72vh] min-h-[34rem] bg-[var(--canvas)] p-4">
        ...
        <output aria-live="polite" className="absolute bottom-7 left-7 rounded-[var(--radius-sm)] bg-[var(--surface)]/95 px-3 py-2 font-mono text-xs font-bold shadow-sm">{cursor ? `x ${cursor.x.toFixed(3)} m · y ${cursor.y.toFixed(3)} m${configuration.snapToGrid ? " · snapped" : ""}` : libraryCategory ? `Click to place ${holdCategoryLabel(libraryCategory)}` : "Origin 0,0 at bottom-left"}</output>
      </div>
    ...
    <aside className="space-y-5"><section className="rounded-[var(--radius-panel)] border border-[var(--border)] bg-[var(--surface)] p-5"><p className="text-xs font-bold uppercase tracking-[.18em] text-[var(--muted)]">Reusable objects · revision {holdsRevision}</p><div className="flex items-center justify-between gap-3"><h2 className="mt-1 text-xl font-black">Hold Library</h2><button className="min-h-10 rounded-[var(--radius-md)] bg-[var(--primary)] px-3 text-sm font-bold text-white disabled:opacity-50" disabled={!holdsDirty || savingHolds} onClick={saveHolds} type="button">{savingHolds ? "Saving…" : holdsDirty ? "Save holds" : "Saved"}</button></div><div className="mt-4 grid grid-cols-2 gap-2">{holdCategories.map((category) => <button aria-pressed={libraryCategory === category} className={`flex min-h-20 flex-col items-center justify-center rounded-[var(--radius-md)] border p-2 text-xs font-bold capitalize focus:outline-none focus:ring-2 focus:ring-[var(--focus-ring)] ${libraryCategory === category ? "border-[var(--primary)] bg-lime-100" : "bg-[var(--surface-subtle)]"}`} draggable key={category} onClick={() => { setLibraryCategory((current) => current === category ? null : category); setTool("select"); }} onDragStart={(event) => { event.dataTransfer.setData("application/x-mycrux-hold", category); event.dataTransfer.effectAllowed = "copy"; }} type="button"><HoldIcon category={category} className="mb-1 h-9 w-9"/>{holdCategoryLabel(category)}</button>)}</div><p aria-live="polite" className={`mt-3 text-sm ${holdsStatus === "error" ? "text-red-700" : holdsStatus === "success" ? "text-emerald-700" : "text-[var(--muted)]"}`}>{holdsMessage}</p></section>

      <section className="rounded-[var(--radius-panel)] border border-[var(--border)] bg-[var(--surface)] p-5" aria-labelledby="selected-hold-heading">
        ...
        {selectedHold ? <div className="mt-4 space-y-4" key={selectedHold.id}>
          <div className="flex items-center gap-3"><span className="grid h-14 w-14 place-items-center rounded-[var(--radius-md)] bg-[var(--surface-subtle)]" style={{ color: selectedHold.metadata.colour }}><HoldIcon category={selectedHold.iconKey} className="h-10 w-10"/></span><div className="min-w-0"><p className="font-black">{selectedHold.metadata.label || holdCategoryLabel(selectedHold.category)}</p><code className="block truncate text-[.65rem] text-[var(--muted)]">{selectedHold.id}</code></div></div>
          ...
          <div className="rounded-[var(--radius-md)] bg-[var(--surface-subtle)] p-3 text-sm"><span className="font-bold">Current wall:</span> {faceName}</div>
          <fieldset className="rounded-[var(--radius-lg)] border border-[var(--border)] p-3"><legend className="px-1 text-sm font-black">Belongs to</legend>{routes.length?<div className="mt-1 max-h-52 space-y-2 overflow-auto">{routes.map((route)=>{const assigned=(routeAssignments[selectedHold.id]??[]).includes(route.id);const archived=route.status==="archived";return <label className={`flex min-h-11 items-center gap-3 rounded-[var(--radius-md)] px-3 py-2 text-sm ${archived?"bg-[var(--surface-subtle)] text-[var(--muted)]":"bg-[var(--surface)] hover:bg-lime-50"}`} key={route.id}><input checked={assigned} disabled={archived} onChange={()=>toggleRouteAssignment(selectedHold.id,route.id)} type="checkbox"/><span className="h-3 w-3 shrink-0 rounded-full border border-[var(--border-strong)]" style={{backgroundColor:route.colour}}/><span className="min-w-0 flex-1 font-bold">{route.name} · {route.grade}</span>{archived?<span className="text-xs">Archived</span>:null}</label>;})}</div>:<p className="mt-1 text-sm text-[var(--muted)]">Create a route before assigning this hold.</p>}<button className="mt-3 min-h-11 w-full rounded-[var(--radius-md)] bg-[var(--primary)] px-3 text-sm font-bold text-white focus:outline-none focus:ring-2 focus:ring-[var(--focus-ring)] focus:ring-offset-2 disabled:opacity-40" disabled={!assignmentsDirty||savingAssignments||holdsDirty} onClick={saveAssignments} type="button">{savingAssignments?"Saving routes…":assignmentsDirty?"Save route assignments":"Route assignments saved"}</button>{holdsDirty&&assignmentsDirty?<p className="mt-2 text-xs text-[var(--muted)]">Save physical hold changes before saving route assignments.</p>:null}</fieldset>
          ...
          <button className="min-h-11 w-full rounded-[var(--radius-md)] border border-red-200 bg-red-50 px-3 text-sm font-bold text-red-800 focus:outline-none focus:ring-2 focus:ring-red-700 focus:ring-offset-2 disabled:opacity-40" disabled={holdsDirty||assignmentsDirty||operatingPhysical} onClick={retireSelectedHold} type="button">Delete / retire hold</button>{holdsDirty||assignmentsDirty?<p className="text-xs text-[var(--muted)]">Save or undo pending changes before replacing or retiring this physical hold.</p>:null}
        </div>:<p className="mt-3 text-sm text-[var(--muted)]">Select a hold on the wall or from the installed list.</p>}
      </section>

      <section className="rounded-[var(--radius-panel)] border border-[var(--border)] bg-[var(--surface)] p-5"><div className="flex items-center justify-between"><h2 className="text-xl font-black">Installed holds</h2><span className="text-sm font-bold">{holds.length}</span></div>{holds.length ? <ol className="mt-3 max-h-64 space-y-2 overflow-auto">{holds.map((hold, index) => <li key={hold.id}><button className={`flex min-h-11 w-full items-center gap-3 rounded-[var(--radius-md)] px-3 text-left text-sm focus:outline-none focus:ring-2 focus:ring-[var(--focus-ring)] ${hold.id === selectedHoldId ? "bg-lime-100" : "bg-[var(--surface-subtle)]"}`} onClick={() => { setSelectedHoldId(hold.id); setLibraryCategory(null); setTool("select"); }} type="button"><span style={{ color: hold.metadata.colour }}><HoldIcon category={hold.iconKey} className="h-7 w-7"/></span><span className="min-w-0 flex-1 truncate font-bold">{hold.metadata.label || `${holdCategoryLabel(hold.category)} ${index + 1}`}</span><span className="text-xs tabular-nums text-[var(--muted)]">{hold.position.x.toFixed(2)}, {hold.position.y.toFixed(2)}</span></button></li>)}</ol> : <p className="mt-3 rounded-[var(--radius-md)] bg-[var(--surface-subtle)] p-4 text-sm text-[var(--muted)]">Drag a reusable icon onto the wall to create the first hold.</p>}</section>

      <section className="rounded-[var(--radius-panel)] border border-[var(--border)] bg-[var(--surface)] p-5"><p className="text-xs font-bold uppercase tracking-[.18em] text-[var(--muted)]">Canvas · revision {revision}</p><h2 className="mt-1 text-xl font-black">{faceName}</h2><p className="text-sm text-[var(--muted)]">{structureName} · {widthMetres} × {heightMetres} m · {angleDegrees}°</p><label className="mt-4 block text-sm font-bold">Snap interval (m)<input className={inputClass} max="5" min="0.05" onChange={(event) => changeSettings({ gridSizeMetres: Number(event.target.value) })} step="0.05" type="number" value={configuration.gridSizeMetres}/></label><div className="mt-4 space-y-3"><label className="flex items-center gap-3 text-sm font-bold"><input checked={configuration.showGrid} onChange={(event) => changeSettings({ showGrid: event.target.checked })} type="checkbox"/>Show grid</label><label className="flex items-center gap-3 text-sm font-bold"><input checked={configuration.snapToGrid} onChange={(event) => changeSettings({ snapToGrid: event.target.checked })} type="checkbox"/>Snap to grid</label></div><button className="mt-4 min-h-11 w-full rounded-[var(--radius-md)] bg-[var(--primary)] px-4 text-sm font-bold text-white disabled:opacity-50" disabled={!settingsDirty || savingSettings} onClick={saveSettings} type="button">{savingSettings ? "Saving…" : settingsDirty ? "Save canvas settings" : "Settings saved"}</button><p aria-live="polite" className={`mt-3 text-sm ${settingsStatus === "error" ? "text-red-700" : settingsStatus === "success" ? "text-emerald-700" : "text-[var(--muted)]"}`}>{settingsMessage}</p></section></aside>
  </div>;
}
