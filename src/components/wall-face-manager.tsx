...
const fieldClass = "mt-1 min-h-11 w-full rounded-xl border border-[var(--border)] bg-white px-3 py-2 text-sm font-normal focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-2";
const buttonClass = "min-h-10 rounded-xl border border-[var(--border)] px-3 text-sm font-bold focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-2 disabled:opacity-40";
...
export function WallFaceManager({ gymSlug, initialFaces, initialRevisions, persistedStructureIds, structure }: { gymSlug: string; initialFaces: ClimbingFace[]; initialRevisions: Record<string, number>; persistedStructureIds: Set<string>; structure: Structure }) {
  ...
  return <section className="rounded-3xl border border-[var(--border)] bg-white p-5" aria-labelledby="faces-heading">
    <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[var(--muted)]">Phase 2</p><h2 className="mt-1 text-xl font-black" id="faces-heading">Climbing faces</h2></div>{structure ? <span className="rounded-full bg-stone-100 px-3 py-1 text-xs font-bold">{faces.length}</span> : null}</div>
    {!structure ? <p className="mt-3 text-sm text-[var(--muted)]">Select a wall structure to manage its climbing faces.</p> : !persisted ? <div className="mt-4 rounded-xl bg-amber-50 p-4 text-sm text-amber-950"><strong>Save the floorplan first.</strong><p className="mt-1">The wall structure must exist before faces can be attached.</p></div> : <>
