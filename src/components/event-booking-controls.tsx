...
export function EventBookingControls({ gymSlug, eventId, status }: { gymSlug: string; eventId: string; status: string | null }) {
  ...
  if (status === "registered" || status === "waitlisted") return <form action={cancelAction}>
    ...
    <p className="mb-3 rounded-xl bg-emerald-50 p-4 text-sm font-bold">Your status: {status}</p>
    <button className="min-h-11 rounded-full border border-red-700 px-5 text-sm font-bold text-red-700" disabled={cancelling}>{cancelling ? "Cancelling…" : "Cancel registration"}</button>
  ...
  return <form action={registerAction}>
    <input name="gymSlug" type="hidden" value={gymSlug}/><input name="eventId" type="hidden" value={eventId}/>
    <button className="min-h-11 rounded-full bg-black px-5 text-sm font-bold text-white" disabled={registering}>{registering ? "Booking…" : "Book or join waitlist"}</button>
