...
export function GymSwitcher({
  ...
  return (
    <form action={switchGymAction} className="mt-3 flex items-center gap-2">
      ...
      <select className="min-h-10 min-w-0 flex-1 rounded-lg border border-[var(--border)] bg-white px-2 text-sm font-semibold" defaultValue={activeGym.slug} id="active-gym" name="gymSlug">
        {gyms.map((gym) => <option key={gym.id} value={gym.slug}>{gym.name}</option>)}
      </select>
      <input name="destination" type="hidden" value={destination} />
      <button className="min-h-10 rounded-lg bg-[var(--accent)] px-3 text-xs font-bold" type="submit">Switch</button>
    </form>
  );
}
