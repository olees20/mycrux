-- Support Supabase's opaque sb_secret_... keys. PostgREST authorizes these as the
-- service_role database role, but they do not populate the legacy JWT claim GUC.
-- EXECUTE privileges are therefore the authoritative RPC boundary.

create or replace function public.check_administrative_reset_access()
returns boolean
language sql
stable
set search_path = ''
...
revoke all on function public.check_administrative_reset_access() from public, anon, authenticated;
grant execute on function public.check_administrative_reset_access() to service_role;
...
comment on function public.check_administrative_reset_access() is
  'Non-destructive service-role preflight for the local administrative reset tool.';
comment on function public.administrative_reset_application_data() is
  'Destructively truncates MyCrux application data. Authorization is enforced by an exclusive service_role EXECUTE grant compatible with legacy and opaque Supabase secret keys.';
