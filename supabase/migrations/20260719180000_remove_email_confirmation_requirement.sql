-- Email/password registrations now receive a session immediately. Keep every caller,
-- tenant, role and lifecycle check while removing email_confirmed_at as authorization.
...
returns table(state text, gym_id uuid, gym_slug text, gym_name text, invitation_role text)
language plpgsql
stable
security definer
set search_path = ''
...
begin
  ...
  if actor_email is null then
    raise exception 'An email address is required' using errcode = '28000';
  end if;
  ...
  select
    ...
    gym.id, gym.slug, gym.name, invitation.role
  from public.gyms gym where gym.id = invitation.gym_id;
end;
$$;
...
begin
  ...
  insert into public.invitations (gym_id, email, token_hash, role, staff_role_id, invited_by, expires_at)
  values (
...
-- No current policy or function should retain confirmation as an authorization input.
drop function private.is_current_email_verified();
...
comment on function public.create_my_first_gym(jsonb) is
  'Atomically creates one trial gym for the active membership-empty caller and assigns only that caller as owner.';
